...
- *Contract*: Accepts a `Context`; executes seven validation tests sequentially; prints a summary.
...
**Program.Run**: Invoked by all seven internal validation test methods to exercise the full tool
dispatch pipeline end-to-end.

- *Type*: In-process .NET internal API (static method).
- *Role*: Consumer (each of the seven test methods creates a `Context` and passes it to
  `Program.Run`).
...
1. `Validation.Run` prints a header block and then executes each of the seven test methods
...
The seven tests exercise: requirements processing, trace matrix construction, report export,
tag filtering, enforcement mode, orphan detection, and lint detection. Each runs in a dedicated
